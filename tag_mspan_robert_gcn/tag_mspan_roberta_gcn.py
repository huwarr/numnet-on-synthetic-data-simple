    ...
    def __init__(
        ...
        if "multiple_spans" in self.answering_abilities:
            if self.multispan_head_name == "flexible_loss":
                self.multispan_head = multispan_heads_mapping[multispan_head_name](
                    modeling_out_dim,
        ...
        if "multiple_spans" in self.answering_abilities:
            if self.multispan_head_name == "flexible_loss":
                multispan_log_probs, multispan_logits = self._multispan_module(
                    sequence_output, seq_mask=multispan_mask
                )
            else:
                multispan_log_probs, multispan_logits = self._multispan_module(
                    sequence_output
                ...
                if answering_ability == "passage_span_extraction":
                    ...
                    if self.multispan_head_name == "flexible_loss":
                        log_marginal_likelihood_for_multispan = (
                            self._multispan_log_likelihood(
                    ...
                    if predicted_ability_str == "passage_span_extraction":
                        ...
                        if self.multispan_head_name == "flexible_loss":
                            (
                                ...
                                answer_json["spans"],
                                invalid_spans,
                            ) = self._multispan_prediction(
                                multispan_log_probs[i],
                                multispan_logits[i],
                                qp_tokens,
                                passage_str,
                                question_str,
                                multispan_mask[i],
                                bio_wordpiece_mask[i],
                                self.multispan_use_prediction_beam_search
                        ...
                        if self._unique_on_multispan:
                            ...
                            if self._dont_add_substrings_to_ms:
                                answer_json["value"] = remove_substring_from_prediction(
                                    answer_json["value"]
                                )
