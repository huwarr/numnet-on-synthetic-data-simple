...
options.add_finetune_args(parser)
...
def main():
    ...
    if args.pretrained:
        ...
        if args.numeric_only:
            ...
            network._answer_ability_predictor = FFNLayer(
                3 * bert_model.config.hidden_size,
                bert_model.config.hidden_size,
                len(network.answering_abilities),
                args.dropout,
            )
            network._addition_subtraction_index += 1
            network._counting_index += 1
        
        if args.use_gcn:
            node_dim = bert_model.config.hidden_size
            network._gcn_input_proj = nn.Linear(node_dim * 2, node_dim)
            network._gcn = GCN(node_dim=node_dim, iteration_steps=args.gcn_steps)
            network._iteration_steps = args.gcn_steps
            print("gcn iteration_steps=%d" % network._iteration_steps, flush=True)
            network._proj_ln = nn.LayerNorm(node_dim)
            network._proj_ln0 = nn.LayerNorm(node_dim)
            network._proj_ln1 = nn.LayerNorm(node_dim)
            network._proj_ln3 = nn.LayerNorm(node_dim)
            network._gcn_enc = ResidualGRU(bert_model.config.hidden_size, args.dropout, 2)
            network.use_gcn = True
            network.gcn_steps = args.gcn_steps
