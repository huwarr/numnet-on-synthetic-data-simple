...
class DropBatchGen(object):
    def __init__(self, args, data_type, data_mode, tokenizer, padding_idx=1):
        ...
        dpath = "cached_roberta_{}_{}.pkl".format(data_type, data_mode)
        ...
        self.data = DropBatchGen.make_baches(
            all_data,
    ...
    def __iter__(self):
        while self.offset < len(self):
            ...
            max_pans_choice = min(
                8, max([1] + [len(item["answer_passage_spans"]) for item in metas])
            )
            max_qans_choice = min(
                8, max([1] + [len(item["answer_question_spans"]) for item in metas])
            )
            max_sign_choice = max(
            ...
            answer_as_add_sub_expressions = torch.LongTensor(
                bsz, max_sign_choice, max_num_len
            ).fill_(0)
            ...
            for i in range(bsz):
                ...
                if pn_len > 0:
                    number_indices[i, :pn_len] = passage_start + torch.LongTensor(
                        metas[i]["number_indices"][:pn_len]
                    )
                ...
                # answer sign info
                sign_len = min(
                    len(metas[i]["signs_for_add_sub_expressions"]), max_sign_choice
                )
