...
def best_answers_extraction(best_spans, span_num, original_str, offsets, offset_start):
    ...
    for i in range(20):
        start_offset = (
            offsets[predicted_span[i][0] - offset_start][0]
            if predicted_span[i][0] - offset_start < len(offsets)
            else offsets[-1][0]
        )
        end_offset = (
            offsets[predicted_span[i][1] - offset_start][1]
            if predicted_span[i][1] - offset_start < len(offsets)
            else offsets[-1][1]
        )
        ...
        if len(predict_answers) == 0 or all(
            [
                len(set(item.split()) & set(predict_answer.split())) == 0
                for item in predict_answers
            ]
        ):
...
class NumericallyAugmentedBertNet(nn.Module):
    def __init__(
        ...
        if len(self.answering_abilities) > 1:
            self._answer_ability_predictor = FFNLayer(
                3 * hidden_size,
                hidden_size,
        ...
        if "addition_subtraction" in self.answering_abilities:
            ...
            self._number_sign_predictor = FFNLayer(
                5 * hidden_size, hidden_size, 3, dropout_prob
            )

        if "counting" in self.answering_abilities:
            self._counting_index = self.answering_abilities.index("counting")
            self._count_number_predictor = FFNLayer(
                5 * hidden_size, hidden_size, 10, dropout_prob
            )
    ...
    def forward(
        ...
        if (
            ...
            sequence_alg = self._gcn_input_proj(
                torch.cat([sequence_output_list[2], sequence_output_list[3]], dim=2)
            )
            ...
            clamped_number_indices = util.replace_masked_values(
                real_number_indices, number_mask, gcn_info_vec.size(1) - 1
            )
            gcn_info_vec.scatter_(
                1,
                clamped_number_indices.unsqueeze(-1).expand(-1, -1, d_node.size(-1)),
                d_node,
            )
        ...
        if "counting" in self.answering_abilities:
            ...
            count_number_logits = self._count_number_predictor(
                torch.cat(
                    [number_vector, passage_h2, question_h2, sequence_output[:, 0]],
                    dim=1,
                )
            )
        ...
        if (
            ...
            sequence_span_end_logits = self._span_end_predictor(
                sequence_for_span_end
            ).squeeze(-1)
            # Shape: (batch_size, passage_length)

            # span number prediction
            span_num_logits = self._proj_span_num(
                torch.cat([passage_h2, question_h2, sequence_output[:, 0]], dim=1)
            )
            ...
            if "passage_span_extraction" in self.answering_abilities:
                ...
                best_passage_span = get_best_span(
                    passage_span_start_logits, passage_span_end_logits
                )
            ...
            if "question_span_extraction" in self.answering_abilities:
                ...
                best_question_span = get_best_span(
        ...
        if (
            ...
            for answering_ability in self.answering_abilities:
                if answering_ability == "passage_span_extraction":
                    ...
                    gold_add_sub_mask = (
                        answer_as_add_sub_expressions.sum(-1) > 0
                    ).float()
        ...
        if metadata is not None:
            ...
            for i in range(batch_size):
                ...
                if predicted_ability_str == "passage_span_extraction":
                    ...
                    predicted_signs = [
                        sign_remap[it]
                        for it in best_signs_for_numbers[i].detach().cpu().numpy()
                    ]
                    result = sum(
                        [
                            sign * number
                    ...
                    answer_json["number_sign_log_probs"] = (
                        number_sign_log_probs[i, :, :].detach().cpu().numpy()
                    )
